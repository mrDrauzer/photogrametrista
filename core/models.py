...
class ProjectTemplate(models.Model):
    ...
    default_quality = models.CharField(_("Качество по умолчанию"), max_length=20, default='MEDIUM')
    default_target_gsd = models.FloatField(_("Целевой GSD по умолчанию"), null=True, blank=True)
...
class Project(models.Model):
    ...
    share_token = models.CharField(_("Токен доступа"), max_length=100, null=True, blank=True, unique=True)
    share_password = models.CharField(_("Пароль доступа"), max_length=128, null=True, blank=True)
    ...
    processing_quality = models.CharField(_("Качество обработки"), max_length=20, default='MEDIUM')
    ortho_resolution = models.FloatField(_("Разрешение ортофото (см/пикс)"), null=True, blank=True)
    dsm_resolution = models.FloatField(_("Разрешение DSM (см/пикс)"), null=True, blank=True)
...
class Task(models.Model):
    ...
    branch = models.ForeignKey('ProjectBranch', on_delete=models.SET_NULL, null=True, blank=True, related_name='tasks')
    ...
    quality_preset = models.CharField(_("Настройка качества"), max_length=20, default='MEDIUM')
...
class Artifact(models.Model):
    ...
    branch = models.ForeignKey('ProjectBranch', on_delete=models.SET_NULL, null=True, blank=True, related_name='artifacts')
    artifact_type = models.CharField(_("Тип артефакта"), max_length=20, choices=ARTIFACT_TYPES)
...
class Orthophoto(models.Model):
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='orthophotos')
...
class ProjectPresence(models.Model):
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='presence')
...
class MeasuringResult(models.Model):
    ...
    measure_type = models.CharField(_("Тип замера"), max_length=10, choices=MEASURE_TYPES)
...
class Annotation(models.Model):
    ...
    annotation_type = models.CharField(_("Тип"), max_length=20, choices=ANNOTATION_TYPES)
...
class ProjectShare(models.Model):
    ...
    access_level = models.CharField(_("Уровень доступа"), max_length=10, choices=ACCESS_CHOICES, default='VIEW')
    created_at = models.DateTimeField(auto_now_add=True)
...
class Organization(models.Model):
    ...
    primary_color = models.CharField(_("Основной цвет"), max_length=7, default='#2196f3')
    created_at = models.DateTimeField(auto_now_add=True)
...
class UserProfile(models.Model):
    ...
    data_retention_days = models.IntegerField(_("Срок хранения данных (дней)"), default=365)
...
class QualityMetric(models.Model):
    ...
    artifact = models.ForeignKey(Artifact, on_delete=models.CASCADE, related_name='quality_metrics')
...
class SurveyPoint(models.Model):
    survey = models.ForeignKey(SurveyTask, on_delete=models.CASCADE, related_name='points')
...
class WebhookConfig(models.Model):
    ...
    events = models.JSONField(_("Events"), default=list) # e.g. ["task.completed", "artifact.created"]
...
class DetectedObject(models.Model):
    ...
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='detected_objects')
    object_type = models.CharField(_("Тип объекта"), max_length=20, choices=OBJECT_TYPES)
...
class GCP(models.Model):
    ...
    point_type = models.CharField(_("Тип"), max_length=10, choices=GCP_TYPES, default='CONTROL')
...
class FlightPlan(models.Model):
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='flight_plans')
...
class FlightWaypoint(models.Model):
    plan = models.ForeignKey(FlightPlan, on_delete=models.CASCADE, related_name='waypoints')
