...
from .models import (
    ...
    Annotation, ProjectShare, ActivityLog, UserProfile, Organization, QualityMetric, DetectedObject,
    WebhookConfig, GCP, ProjectTemplate, FlightPlan, FlightWaypoint, SurveyPoint, SurveyTask, ProjectBranch, CRSDefinition, ProjectTimeline,
    ProjectCategory, MeasuringResult, Orthophoto
)
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserProfile
        fields = ['dark_mode', 'notifications_enabled', 'quality_preference', 'language', 'data_retention_days', 'organization', 'organization_details']
...
class ProjectFileSerializer(serializers.ModelSerializer):
    class Meta:
        model = ProjectFile
        fields = ['id', 'project', 'name', 'file', 'created_at', 'latitude', 'longitude', 'altitude']
...
class ProjectSerializer(GeoModelSerializer):
    ...
    class Meta:
        model = Project
        fields = ['id', 'name', 'description', 'created_at', 'updated_at', 'area', 'files_count', 'is_public', 'is_archived', 'owner', 'tags', 'template', 'template_details', 'category', 'category_details', 'share_token', 'has_password', 'processing_quality', 'ortho_resolution', 'dsm_resolution', 'optimize_mesh', 'target_gsd']
        ...
        if value:
            ...
            if isinstance(value, dict):
                ...
                if not coords or len(coords[0]) < 4:
                    raise serializers.ValidationError("Polygon must have at least 4 points (including the closing point).")
                try:
...
class ArtifactSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Artifact
        fields = ['id', 'project', 'task', 'artifact_type', 'artifact_type_display', 'file', 'version', 'created_at', 'metadata', 'name']
...
class ProjectShareSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjectShare
        fields = ['id', 'project', 'user', 'user_username', 'access_level', 'created_at']

class MeasuringResultSerializer(serializers.ModelSerializer):
    author_username = serializers.ReadOnlyField(source='author.username')
    class Meta:
        model = MeasuringResult
        fields = ['id', 'project', 'author', 'author_username', 'measure_type', 'value', 'unit', 'created_at']
...
class AnnotationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Annotation
        fields = ['id', 'project', 'author', 'author_username', 'text', 'annotation_type', 'position', 'created_at']
        read_only_fields = ['author']
...
class ActivityLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ActivityLog
        fields = ['id', 'project', 'user', 'user_username', 'action', 'details', 'created_at']
...
class SurveyTaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SurveyTask
        fields = ['id', 'project', 'name', 'inspector', 'inspector_username', 'date', 'notes', 'points', 'created_at']
...
class FlightPlanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FlightPlan
        fields = ['id', 'project', 'name', 'altitude', 'speed', 'overlap_h', 'overlap_v', 'waypoints', 'created_at']
...
class DetectedObjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'project', 'object_type', 'object_type_display', 'confidence', 'location', 'created_at']
...
class OrthophotoSerializer(GeoModelSerializer):
    ...
    class Meta:
        model = Orthophoto
        fields = ['id', 'project', 'name', 'file_path', 'bounds', 'created_at', 'resolution', 'tiles_url', 'file_size']
