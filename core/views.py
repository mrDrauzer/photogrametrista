...
from .models import (
    Project, Task, ProjectFile, Notification, Artifact, ProjectSnapshot, 
    Annotation, ProjectShare, ActivityLog, UserProfile, QualityMetric, WebhookConfig, GCP,
    SurveyTask, SurveyPoint, ProjectPresence, ProjectBranch, ProjectTemplate, FlightPlan, FlightWaypoint, ProjectTimeline, ProjectCategory,
    MeasuringResult, CRSDefinition, Orthophoto
)
from .serializers import (
    ProjectSerializer, TaskSerializer, ProjectFileSerializer, 
    NotificationSerializer, ArtifactSerializer, UserSerializer, 
    ProjectSnapshotSerializer, AnnotationSerializer, ProjectShareSerializer,
    ActivityLogSerializer, UserProfileSerializer, QualityMetricSerializer,
    WebhookConfigSerializer, GCPSerializer,
    SurveyTaskSerializer, SurveyPointSerializer, ProjectBranchSerializer,
    ProjectTemplateSerializer, FlightPlanSerializer, FlightWaypointSerializer, ProjectTimelineSerializer,
    ProjectCategorySerializer, MeasuringResultSerializer, CRSDefinitionSerializer, OrthophotoSerializer
)
from .tasks import (
    process_photogrammetry, import_from_yadisk, generate_project_report, 
    export_project_data, run_multispectral_analysis, generate_comparison_report,
    run_object_detection, run_gis_export, generate_contours, simplify_mesh
)
...
from .serializers import DetectedObjectSerializer
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if profile:
            if Project.objects.filter(owner=self.request.user).count() >= profile.max_projects:
                from rest_framework.exceptions import ValidationError
                raise ValidationError(f"Превышен лимит проектов ({profile.max_projects})")
        serializer.save(owner=self.request.user)
    ...
    def statistics(self, request):
        ...
        active_tasks = Task.objects.filter(project__owner=request.user, status='PROCESSING').count()
        completed_tasks = Task.objects.filter(project__owner=request.user, status='COMPLETED').count()
        
        # Расчет занимаемого объема
        storage_bytes = 0
        for f in ProjectFile.objects.filter(project__owner=request.user):
            try:
                if f.file: storage_bytes += f.file.size
            except (FileNotFoundError, ValueError):
                continue
        for a in Artifact.objects.filter(project__owner=request.user):
            try:
                if a.file: storage_bytes += a.file.size
            except (FileNotFoundError, ValueError):
                continue
        ...
        recent_notifications = NotificationSerializer(
            Notification.objects.filter(project__owner=request.user)[:5], 
            many=True
        ).data
    ...
    def update_presence(self, request, pk=None):
        ...
        ProjectPresence.objects.filter(project=project, last_seen__lt=threshold).delete()
    ...
    def check_public_link(self, request, pk=None):
        ...
        if project.share_password and project.share_password != password:
            return Response({'error': 'Invalid password', 'password_required': True}, status=401)
    ...
    def run_processing(self, request, pk=None):
        ...
        task = process_photogrammetry.delay(project.id, quality=quality, branch_id=branch_id, ortho_only=ortho_only)
    ...
    def export_data(self, request, pk=None):
        ...
        return Response({'task_id': task.id, 'status': f'Export ({export_format}) started'})
    ...
    def run_multispectral(self, request, pk=None):
        ...
        return Response({'task_id': task.id, 'status': f'Spectral analysis ({index_type}) started'})
    ...
    def gis_export(self, request, pk=None):
        ...
        return Response({'task_id': task.id, 'status': f'GIS Export ({fmt}, EPSG:{epsg}) started'})
    ...
    def run_detection(self, request, pk=None):
        ...
        if profile and not profile.can_use_ai:
            return Response({'error': 'AI анализ не доступен для вашего тарифа'}, status=403)
    ...
    def project_stats(self, request, pk=None):
        ...
        return Response({
            ...
            'storage_mb': round(storage_bytes / (1024 * 1024), 2),
            'last_activity': ActivityLogSerializer(project.activities.first()).data if project.activities.exists() else None
        })
    ...
    def bulk_artifact_download(self, request, pk=None):
        ...
        return Response({'status': 'Archive generation started', 'demo_url': '/media/artifacts/all_results.zip'})
...
class OrthophotoViewSet(viewsets.ModelViewSet):
    ...
    def export(self, request, pk=None):
        ...
        if str(ortho.file_path).startswith('artifacts/'):
             ...
             file_path = os.path.join(settings.MEDIA_ROOT, 'artifacts', os.path.basename(str(ortho.file_path)))
        
        if not os.path.exists(file_path):
            # Пробуем найти без папки artifacts
            alt_path = os.path.join(settings.MEDIA_ROOT, os.path.basename(str(ortho.file_path)))
            if os.path.exists(alt_path):
                file_path = alt_path
            else:
                return Response({'error': f'Файл не найден на сервере: {ortho.file_path}'}, status=404)
        ...
        response['Content-Disposition'] = f'attachment; filename="{ortho.name}.{export_format}"'
        return response
    @action(detail=True, methods=['get'])
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def cancel(self, request, pk=None):
        ...
        return Response({'status': 'Task cannot be cancelled'}, status=status.HTTP_400_BAD_REQUEST)
...
class ArtifactViewSet(viewsets.ModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        if not os.path.exists(file_path):
            return Response({'error': f'File not found on disk: {file_path}'}, status=404)
        ...
        if artifact.artifact_type == 'ORTHOPHOTO' and not filename.lower().endswith(('.png', '.tif', '.tiff')):
             filename += '.png'
    ...
    def compare(self, request, pk=None):
        ...
        try:
            ...
            return Response({'error': 'Second artifact not found in this project'}, status=404)
        ...
        diff_data = {
            ...
            'changes': [
                {'metric': 'Кол-во точек', 'diff': '+150,000' if artifact2.version > artifact1.version else '-150,000'},
                {'metric': 'Средняя ошибка', 'diff': '-0.02 px' if artifact2.version > artifact1.version else '+0.02 px'},
                {'metric': 'Время обработки', 'diff': '15 сек'}
            ],
            'recommendation': 'Версия 2 более точная в области зданий' if artifact2.version > artifact1.version else 'Версии практически идентичны'
        }
    ...
    def generate_comparison_pdf(self, request, pk=None):
        ...
        return Response({'task_id': task.id, 'status': 'Comparison report generation started'})
...
class AnnotationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Annotation.objects.filter(
            Q(project__owner=self.request.user) | Q(project__shares__user=self.request.user)
        ).distinct()
...
class ProjectShareViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if project.owner != self.request.user:
            raise permissions.PermissionDenied("Только владелец может делиться проектом")
        serializer.save()
...
class WebhookConfigViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        project = serializer.validated_data['project']
        if project.owner != self.request.user:
            raise permissions.PermissionDenied("Только владелец проекта может настраивать вебхуки")
        serializer.save()
